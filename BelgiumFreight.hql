...
--Cumulative Traffic Export

USE freight_group;

INSERT OVERWRITE DIRECTORY '/user/mbuard/freight_output/cumulative_segments_by_day'
ROW FORMAT DELIMITED
FIELDS TERMINATED BY ','
SELECT
  DATE(ts) AS day,
  time_segment,
  time_segment_sort,
  SUM(traffic_count) AS segment_traffic,
  SUM(SUM(traffic_count)) OVER (
    PARTITION BY DATE(ts)
    ORDER BY time_segment_sort
    ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW
  ) AS cumulative_segment_traffic
FROM traffic_enriched_clean
GROUP BY DATE(ts), time_segment, time_segment_sort
ORDER BY day, time_segment_sort;
